...
1. 协议与环境层 (Protocol & Config) - app/config.py & .env
设计理念：系统的“物理常数”与安全边界。

核心功能：统一管理存储路径、模型参数（Local LLM 接入地址）、Chunk 尺寸规范以及我们刚刚达成的 “异常处理协议”。它是所有层级的共同语言，确保了开发环境与生产环境的无缝切换。

2. 感知层 (Perception) - services/watcher_service.py
设计理念：建立“活体化”知识库。

核心功能：内置 Sentinel（哨兵） 机制，毫秒级感应物理磁盘变动。无论是 UI 上传还是后端直接操作，系统都能自动识别目录结构并维护资产状态。

3. 流水线层 (Pipeline) - pipeline/ingest.py & services/ingest_service.py
设计理念：切片的“微米级”手术与计算负载均衡。

核心功能：采用 800-1200 字符动态窗口及 15% 语义重叠。具备“并发分批”处理能力，通过 “捕获即脱水” 协议确保在大规模资产注入时，系统依然保持逻辑链路的绝对稳定。

4. 内核层 (Core) - core/engine.py
设计理念：物理级的“认知域隔离”，隔离逻辑完全由用户掌控。

核心功能：核心 RAG 引擎，强制注入 Domain 标签过滤。在数据库底层实现物理墙，确保“行政”不看“财务”，实现受控的“上帝视角”切换。

5. 交互层 (Presentation) - app_ui.py
设计理念：2026 标准下的“冷热隔离”契约化交互。
...
**Matrix Intelligence 核心安全规范：纵向贯穿体系**
...
* 所在层级：Core (内核层)
* 规范内容：强制在向量检索（Retrieval）时注入 filter={'domain': selected_domain}。
* 工程目的：确保 AI 的认知被物理文件夹边界锁死。即使所有数据都在同一个 FAISS 索引里，财务的 AI 助手也绝对无法“越界”检索到行政的敏感文件。

**3. 负载安全：分批吞吐限流 (Batch Load Control)**
* 所在层级：Pipeline (流水线层)
* 规范内容：设定 BATCH_SIZE 阈值，禁止全量数据一次性冲击显存/内存。
* 工程目的：针对本地化 LLM 环境，防止因资产过大导致系统 OOM（内存溢出）或死机。

**4 交互安全：冷热隔离拦截 (Pre-flight Interception)**
* 所在层级：Presentation (交互层)
* 规范内容：启动时执行“物理预检”，若无索引则强制保持 None。
* 工程目的：将重度计算（Ingest）的启动权完全交给用户。防止系统在未授权的情况下自动执行高能耗、高耗时的扫描任务，确保 UI 的极致响应性。

****5.状态安全**：单例模式锁定 (Singleton Enforcement)**
* 所在层级：Service (服务调度)
* 规范内容：通过 _CACHED_VECTORSTORE 全局变量锁定唯一内存实例。
* 工程目的：防止 Streamlit 的 Rerun 机制导致重复加载模型和索引，从根源上杜绝内存泄漏和日志重复打印。
